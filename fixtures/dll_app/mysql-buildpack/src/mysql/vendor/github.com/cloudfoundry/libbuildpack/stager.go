...
func (s *Stager) AddBinDependencyLink(destPath, sourceName string) error {
	...
	// needs to be put into a _windows.go file
...
func (s *Stager) SetStagingEnvironment() error {
	for envVar, dir := range stagingEnvVarDirs {
		...
		if len(depsPaths) != 0 {
			...
			//TODO: should be changed to ; but need multiple depsPath to confirm
...
func (s *Stager) SetLaunchEnvironment() error {
	...
	for envVar, dir := range launchEnvVarDirs {
		...
		if len(depsPaths) != 0 {
			...
			//                             set PATH=%DEPS_DIR%\0\bin;%DEPS_DIR%\1\bin;%PATH%
			scriptContents += fmt.Sprintf(`set %[1]s=%[2]s;%%%[1]s%%`, envVar, strings.Join(depsPaths, ";"))
			scriptContents += "\n"
		}
	}
	...
	// move to _windows.go
	scriptLocation := filepath.Join(s.ProfileDir(), "000_multi-supply.bat")
